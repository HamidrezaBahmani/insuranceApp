...
if ("function" !== typeof fdef) var fdef = function(b) { return "function" === typeof b };
if (!fdef(A$)) var A$ = function(b) { return document.getElementById(b) };
if (!fdef(A$$)) var A$$ = function(b, a) { return b.getElementsByTagName(a) };
if (!fdef(cE)) var cE = function(b) { return document.createElement(b) };
if (!fdef(cTN)) var cTN = function(b) { return document.createTextNode(b) };
AMIB.persianCalendar = function(b, a) {
    var c = { inputID: "", extraInputID: "", extraInputFormat: "JD", initialDate: "", defaultDate: "", autoCreateButton: !0, divPickerClassName: "picker", btnClassName: "pcalBtn", onchange: !1 },
        d;
    this.getOption = function(a) { return c[a] };
    ...
    if (c.autoCreateButton) {
        ...
        var g = this;
        e.onclick = function(a) { g.showHidePicker(a); return !1 }
    }
    var f = this;
    d.ondblclick = function(a) { f.showHidePicker(a); return !1 };
    var h = this;
    d.onblur = function() { h.normalizeUserInput() };
    var k = this;
    addEvt(d, "keydown", function(a) { k.keyDownHandler(a, k) });
...
AMIB.persianCalendar.prototype.showHidePicker = function(b) {
    var a = this.inputID + "_pDatePicker",
        c;
    if (c = A$(a)) { if (c.parentNode.removeChild(c), document.onclick = this.oldDocumentOnClick, lastOpenPDatePicker = this.oldDocumentOnClick = !1, b = A$(lastOpenPDateMonthYearID)) b.parentNode.removeChild(b), lastOpenPDateMonthYearID = !1 } else if (!0 === this.enabled) {
        var d = this.input;
        c = cE("div");
        c.id = a;
        ...
        var e = this,
            g = b;
        document.onclick = function() { e.showHidePicker(g) }
    }
};
AMIB.persianCalendar.prototype.createDateTable = function(b, a, c) {
    ...
    var m = this,
        n = a[0] - 1,
        p = a[1];
    f.onclick = function(a) { m.changePickerDate(a, n, p); return !1 };
    e.appendChild(f);
    f = cE("a");
    f.appendChild(cTN("<"));
    ...
    var q = this,
        r = 1 == a[1] ? a[0] - 1 : a[0],
        s = 1 == a[1] ? 12 : a[1] - 1;
    f.onclick = function(a) { q.changePickerDate(a, r, s); return !1 };
    e.appendChild(f);
    g.appendChild(e);
    e = cE("div");
    e.className =
    ...
    var t = this,
        u = a[0] + 1,
        v = a[1];
    f.onclick = function(a) { t.changePickerDate(a, u, v); return !1 };
    e.appendChild(f);
    f = cE("a");
    f.appendChild(cTN(">"));
    ...
    var w = this,
        x = 12 == a[1] ? a[0] + 1 : a[0],
        y = 12 == a[1] ? 1 : a[1] + 1;
    f.onclick = function(a) { w.changePickerDate(a, x, y); return !1 };
    e.appendChild(f);
    g.appendChild(e);
    f = cE("a");
    f.className =
    ...
    var z = this,
        A = a[1],
        B = a[0];
    f.onclick = function(a) { z.showMonthPicker(a, A, B); return !1 };
    g.appendChild(f);
    f = cE("a");
    f.className = "monYear";
    ...
    var C = this,
        E = a[1],
        F = a[0];
    f.onclick = function(a) { C.showYearPicker(a, E, F); return !1 };
    g.appendChild(f);
    e = d.insertRow(-1);
    h = 0;
    ...
    var G = this,
        H = j.join("/");
    c.onclick = function() { G.fillDateField(H); return !1 };
...
AMIB.persianCalendar.prototype.showMonthPicker = function(b, a, c) {
    ...
    if (A$(this.inputID + "_pDatePicker")) {
        ...
        for (d = 0; 12 > d; ++d) g = cE("a"), g.href = "#", g.appendChild(cTN(PERSIAN_MONTH_NAMES[d])), d == a - 1 && (g.className = "selected"), g.onclick = function(a, b, c) { return function(d) { a.monthYearPickerClick(d, b, c); return !1 } }(this,
...
AMIB.persianCalendar.prototype.showYearPicker = function(b, a, c) {
    ...
    if (A$(this.inputID + "_pDatePicker")) {
        ...
        e.className = "monthYearPicker";
        e.id = lastOpenPDateMonthYearID = this.inputID + "monthYearPicker";
        for (d = c - 5; d <= c + 5; ++d) g = cE("a"), g.href = "#", g.appendChild(cTN(enDigitsToFa(d))), d == c && (g.className = "selected"), g.onclick = function(a, b, c) { return function(d) { a.monthYearPickerClick(d, b, c); return !1 } }(this,
            a, d), e.appendChild(g);
        document.body.appendChild(e);
        this.setMenuPos(e, b.target || b.srcElement);
        e.style.zIndex = ++pDatePickerZIndex
...
AMIB.persianCalendar.prototype.setMenuPos = function(b, a) { var c = getOffset(a);
...
AMIB.persianCalendar.prototype.normalizeUserInput = function() { var b = this.input,
        a, c = b.value.trim(),
        c = faDigitsToEn(c);
...
AMIB.persianCalendar.prototype.keyDownHandler = function(b, a) {
    ...
    if (!(48 <= c && 57 >= c || 96 <= c && 105 >= c || 191 === c || 111 === c || 8 === c || 9 === c || 13 === c || 46 === c || 109 === c || 35 === c || 36 === c || 37 === c || 39 === c))
        if (38 === c || 40 === c) { var d = isPersianDate(a.input.value);!1 !== d && (38 === c ? a.fillDateField(jd_to_persian(persian_to_jd(d[0], d[1], d[2]) + 1)) : 40 === c && a.fillDateField(jd_to_persian(persian_to_jd(d[0], d[1], d[2]) - 1)), a.input.select(), prvDef(b)) } else b.ctrlKey ||
            prvDef(b)
};
...
function longYear(b) { var a, c; if (100 <= b) return b;
    a = new Date;
    a = gregorian_to_jd(a.getFullYear(), a.getMonth() + 1, a.getDate());
    a = jd_to_persian(a);
    c = 100 * Math.floor(a[0] / 100);
    70 < Math.abs(a[0] - c - b) && (c += 100); return c + b }
var GREGORIAN_EPOCH = 1721425.5,
    PERSIAN_EPOCH = 1948320.5;
if (!fdef(mod)) var mod = function(b, a) { return b - a * Math.floor(b / a) };

function leap_persian(b) { return 682 > 682 * ((b - (0 < b ? 474 : 473)) % 2820 + 512) % 2816 }
...
function jd_to_persian(b) { var a, c, d;
    ...
    b = b - persian_to_jd(a, c, 1) + 1; return [a, c, b] }

function persian_to_jd(b, a, c) { var d;
    b -= 0 <= b ? 474 : 473;
    d = 474 + mod(b, 2820); return c + (7 >= a ? 31 * (a - 1) : 30 * (a - 1) + 6) + Math.floor((682 * d - 110) / 2816) + 365 * (d - 1) + 1029983 * Math.floor(b / 2820) + (PERSIAN_EPOCH - 1) }

function leap_gregorian(b) { return 0 == b % 4 && !(0 == b % 100 && 0 != b % 400) }

function jd_to_gregorian(b) { var a, c, d, e;
    ...
    b = b - gregorian_to_jd(c, a, 1) + 1; return [c, a, b] }

function gregorian_to_jd(b, a, c) { return GREGORIAN_EPOCH - 1 + 365 * (b - 1) + Math.floor((b - 1) / 4) + -Math.floor((b - 1) / 100) + Math.floor((b - 1) / 400) + Math.floor((367 * a - 362) / 12 + (2 >= a ? 0 : leap_gregorian(b) ? -1 : -2) + c) }

function persianMonthDays(b, a) { return 6 >= a ? 31 : 11 >= a ? 30 : 12 == a ? leap_persian(b) ? 30 : 29 : 0 }
if (!fdef(stopPropg)) var stopPropg = function(b) { b.stopPropagation ? b.stopPropagation() : b.cancelBubble = !0 };
if (!fdef(prvDef)) var prvDef = function(b) { b.preventDefault ? b.preventDefault() : b.returnValue = !1 };
if (!fdef(faDigitsToEn)) var faDigitsToEn = function(b) { for (var a = b.length, c = "", d = 0, d = 0; d < a; ++d) c += String.fromCharCode(1776 <= b.charCodeAt(d) && 1785 >= b.charCodeAt(d) ? b.charCodeAt(d) - 1728 : b.charCodeAt(d)); return c };
if (!fdef(enDigitsToFa)) var enDigitsToFa = function(b) { b = b.toString(); for (var a = b.length, c = "", d = 0, d = 0; d < a; ++d) c += String.fromCharCode(48 <= b.charCodeAt(d) && 57 >= b.charCodeAt(d) ? b.charCodeAt(d) + 1728 : b.charCodeAt(d)); return c };
"trim" in String.prototype || (String.prototype.trim = function() { return this.replace(/^\s*/, "").replace(/\s*$/, "") });
"map" in Array.prototype || (Array.prototype.map = function(b, a) { for (var c = Array(this.length), d = 0, e = this.length; d < e; d++) d in this && (c[d] = b.call(a, this[d], d, this)); return c });
if (!fdef(removeCssClass)) var removeCssClass = function(b, a) { return b.split(/\s+/).map(function(b) { return b === a ? "" : b }).join(" ") };
if (!fdef(getOffset)) var getOffset = function(b) { for (var a = 0, c = 0; b && !isNaN(b.offsetLeft) && !isNaN(b.offsetTop);) a += b.offsetLeft, c += b.offsetTop, b = b.offsetParent; return { top: c, left: a } };
if (!fdef(zeroPad)) var zeroPad = function(b, a) { var c = Math.abs(b),
...
if (!fdef(getScrollTop)) var getScrollTop = function(b) { var a = 0; if ("undefined" !== typeof b) { for (;
            (b = b.parentNode) && "BODY" !== b.nodeName && "undefined" !== typeof b.scrollTop;) a += b.scrollTop; return a } if ("undefined" !== typeof pageYOffset) return pageYOffset;
    b = document.body;
    D = document.documentElement;
    D = D.clientHeight ? D : b; return D.scrollTop };
if (!fdef(addEvt)) var addEvt = function(b, a, c) { b.addEventListener ? b.addEventListener(a, c, !1) : b.attachEvent && b.attachEvent("on" + a, c) };